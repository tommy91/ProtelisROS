...
echo -e "Launching SITL: (I$INSTANCE)\n"
...
log_file=$LOG_DIRECTORY$INSTANCE.log
...
echo -n "Set 'SYSID_THISMAV=$SYSTEM_ID.000000' in the param file.. "
echo "SYSID_THISMAV    $SYSTEM_ID.000000" > $sysid_param_filepath
echo -e "ok\n"

## Build and execute the arducopter command and log its output to file
## such that mavproxy can have the screen to itself
arducopter_cmd="$arducopter_path -S -I$INSTANCE --model $MODEL --speedup 1 --defaults $params_to_use"
